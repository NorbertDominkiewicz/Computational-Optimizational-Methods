...
public class BipartiteController implements ViewController, MethodController {
    ...
    public void onCalculate() {
        ...
        try {
            ...
            if (e.isEmpty() && a.isEmpty() && b.isEmpty() && equation.isEmpty()) {
                ...
                bipartiteResult = bipartiteService.calculate();
                updateXBounds((double) -6, (double) -1);
                updateYBounds((double) - 220, (double) 30);
                loadPoints(bipartiteResult.points(), bipartiteResult.result(),bipartiteResult.fx());
            }
            else {
                if (e.isEmpty()) {
                    bipartiteResult = bipartiteService.calculate(Double.parseDouble(a), Double.parseDouble(b), equation);
                }
                if (e.isEmpty() && a.isEmpty() && b.isEmpty()) {
                    bipartiteResult = bipartiteService.calculate(equation);
                }
                if (!(e.isEmpty() && a.isEmpty() && b.isEmpty() && equation.isEmpty())) {
                    bipartiteResult = bipartiteService.calculate(Double.parseDouble(a), Double.parseDouble(b), Double.parseDouble(e), equation);
                }
                updateXBounds(Double.parseDouble(a), Double.parseDouble(b));
                updateYBounds(null, bipartiteResult.fx());
            }
            swapComponent(Component.RESULT);
            ResultController resultController = (ResultController) components.get(Component.RESULT);
            resultController.addResults(bipartiteResult);
            mainController.addRecentResult("Metoda siecznych: ", bipartiteResult.fx());
            if (bipartiteResult == null) {
